...
class BetheBloch:
    ...
    def ke_at_length(self, ke0, track_len):
        ...
        if self.ke_map.get(int(ke0)) is None:
            self.create_spline_at_ke(int(ke0))
    ...
    def mean_dedx(self, ke):
        ...
        dedx = (self.rho * self.K * self.Z * self.charge*self.charge) / (self.A * beta*beta) * (
                    ...
                    - self.density_effect(beta=beta, gamma=gamma) / 2)

        return dedx

    def integrated_dedx(self, ke0, ke1, n):

        ke0, ke1 = (ke0, ke1) if ke0 > ke1 else (ke1, ke0)

        step_size = (ke1 - ke0) / n

        int_step = np.arange(n)
        dedx = self.mean_dedx(ke0 + (int_step + 0.5) * step_size)
        area = np.nansum(1. / (dedx * step_size))
    ...
    def create_spline_at_ke(self, ke0):
        ...
        calc_points = np.arange(num_points)

        delta_e = []
        track_len = []
        if num_points > 1:
            for pt in range(num_points):
                ke = ke0 - calc_points * 10
                delta_e.append(ke0 - ke)
                track_len.append(self.integrated_dedx(ke0=ke, ke1=ke0, n=50))
        else:
            delta_e = [0, ke0]
            track_len = [0, self.integrated_dedx(ke0=0, ke1=ke0, n=50)]
    ...
    def density_effect(beta, gamma):
        ...
        cond_list = [x >= lar_x1, lar_x0 <= x < lar_x1]
