...
from ROOT import TMath, TGraph, TGraphErrors, TFile, TH1D
import numpy as np
import json
...
class CexDDCrossSection:
    ...
    def extract_cross_section(self, events, event_mask, total_incident_pion):
        """
        ...
        if self.config["run_truth_xsec"]:
            truth_3d_hist = self.run_truth_cross_section(events=events, event_mask=event_mask)
    ...
    def run_truth_cross_section(self, events, event_mask):
        ...
        return truth_xsec.extract_truth_xsec(events=events[event_mask])

    def run_reco_cross_section(self, events, event_mask):
        reco_xsec = RecoCrossSection(self.config)
        return reco_xsec.extract_reco_xsec(events=events[event_mask])
    ...
    def configure(self, config_file):
        ...
        beam_bins = [1000, 1400., 2200.]
        beam_energy_hist = TH1D("beam_energy", "Beam Pi+ Kinetic Energy;T_{#pi^{+}} [MeV/c];Count", len(beam_bins), beam_bins)

        geant_file = "geant_xsec.root"
        ...
        for bin_i in range(1, beam_energy_hist.GetXaxis().GetNbins()+1):
            ...
            geant_graph_name = "inel_cex_" + str(bin_center) + "_MeV"
            print("Loading GEANT Xsec TGraph", geant_graph_name)
            self.geant_xsec_dict[bin_center] = geant_xsec_file.Get(geant_graph_name)
