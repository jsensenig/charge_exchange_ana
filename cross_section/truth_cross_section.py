...
class TruthCrossSection:
    ...
    def beam_end_ke(self, events):
        ...
        return 1000. * (np.sqrt(np.square(pi_mass, 2) + np.square(events[beam_end_momentum], 2)) - pi_mass)
    ...
    def pi0_start_ke(self, events):
        ...
        return 1000. * (np.sqrt(np.square(pi0_mass, 2) + np.square(events[daughter_start_momentum, pi0_daughter_mask], 2))
                        - pi0_mass)
